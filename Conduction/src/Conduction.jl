...
DOS(semiconductor::Semiconductor, U::AbstractFloat, T::AbstractFloat)::Float64 = (semiconductor.Ni / semiconductor.SigmaI(T) * exp(-(U * semiconductor.k * T - semiconductor.ModeEffect)^2 / (2 * semiconductor.SigmaI(T)^2)) + semiconductor.Nd / semiconductor.SigmaD(T) * exp(-(U * semiconductor.k * T - semiconductor.ModeEffect + semiconductor.Ed)^2 / (2 * semiconductor.SigmaD(T)^2))) / sqrt(2 * pi)

# Fermi-Dirac distribution
F(semiconductor::Semiconductor, U::AbstractFloat, T::AbstractFloat)::Float64 = 1 / (1 + exp(U - (semiconductor.ModeEffect + semiconductor.Uf(T)) / (semiconductor.k * T)))

# Number of free state within a sphere of radius R
N(semiconductor::Semiconductor, U::AbstractFloat, T::AbstractFloat, R::AbstractFloat)::Float64 = (semiconductor.k * T) / (8 * semiconductor.alpha^3) * 2 * pi * hcubature(
...
RnnVRH(semiconductor::Semiconductor, U::AbstractFloat, T::AbstractFloat)::Float64 = find_zero(r -> N(semiconductor, U, T, r) - 1, 5, Order0())

# Range to the nearest neighbour using a percolation approach
RnnPerco(semiconductor::Semiconductor, U::AbstractFloat, T::AbstractFloat)::Float64 = ( (4pi) / (3 * 2.8) * quadgk(
...
RnnPercoField(semiconductor::Semiconductor, U::AbstractFloat, T::AbstractFloat)::Float64 = find_zero(r -> N(semiconductor, U, T, r) - 2.8, 5, Order0())

# Effective distance of jump of an electron
function xf(semiconductor::Semiconductor, Rnn::Float64, U::AbstractFloat, T::AbstractFloat)
...
function asymptoteRnnPerco(semiconductor::Semiconductor, U::AbstractFloat, T::AbstractFloat)::Float64
...
function t(semiconductor, Rnn::Float64, U::AbstractFloat, T::AbstractFloat)::Float64
    beta = semiconductor.beta(T)
